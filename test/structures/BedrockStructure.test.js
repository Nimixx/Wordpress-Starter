...
describe('BedrockStructure', () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(bedrockStructure, 'writeProjectFile')
      .mockImplementation((path, content) => ({ path, content }));

    // Mock the actual implementation of these methods
    jest.spyOn(bedrockStructure, 'generateManually').mockImplementation(() => Promise.resolve());

    jest
      .spyOn(bedrockStructure, 'generateWithComposer')
      .mockImplementation(() => Promise.resolve());

    jest.spyOn(bedrockStructure, 'setupIconPackages').mockImplementation(() => Promise.resolve());

    jest.spyOn(bedrockStructure, 'setupSageTheme').mockImplementation(() => Promise.resolve());

    jest
      .spyOn(bedrockStructure, 'setupIconPackagesInTheme')
      .mockImplementation(() => Promise.resolve());
  ...
  describe('setupSageTheme', () => {
    ...
    test('should install Sage theme when user confirms', async () => {
      ...
      bedrockStructure.setupSageTheme = async function () {
        ...
        mockExecSync(`composer create-project roots/sage ${_themeNamePrompt.themeName}`, {
          stdio: 'inherit',
        });
    ...
    test('should handle errors during Sage theme installation', async () => {
      ...
      expect(ui.displayWarning).toHaveBeenCalledWith(
        expect.stringContaining('Error installing Sage theme'),
      );
  ...
  describe('setupIconPackagesInTheme', () => {
    ...
    test('should install icon package in theme when user confirms', async () => {
      ...
      expect(mockExecSync).toHaveBeenCalledWith('composer require blade-ui-kit/blade-heroicons', {
        stdio: 'inherit',
      });
...
NONCE_SALT='generateme'
        ...
        const generateRandomString = () => {
          return 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()-_'.repeat(
            2,
          );
        };
